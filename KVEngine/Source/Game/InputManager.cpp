...
{
	void SteelBattalion::InputManager::updateLoop( InputManager *sbc )
    {
	    sbc->poll();
    }

	SteelBattalion::InputManager::InputManager()
    {
	    ctx = nullptr;
	    device = nullptr;
	...
	unsigned char*SteelBattalion::InputManager::getRawLEDData()
    {
    ...
    void SteelBattalion::InputManager::setRawLEDData(unsigned char *data)
    {
	    std::unique_lock<std::mutex> lock(mtx);
	    memcpy(rawLEDData, data, 34);
    }

    unsigned char*SteelBattalion::InputManager::getRawControlData()
    {
	    std::unique_lock<std::mutex> lock(mtx);
...
void SteelBattalion::InputManager::init()
{
...
void SteelBattalion::InputManager::poll()
{
	uint8_t *temp;
...
void SteelBattalion::InputManager::setPollingInterval(const uint32_t &i)						// This method changes the polling interval. It's not used yet
{
	std::unique_lock<std::mutex> lock(mtx);
	pollingInterval = i;
}

uint32_t SteelBattalion::InputManager::getPollingInterval()						// This method returns the polling interval. This works, but it isn't very accurate because it doesn't poll much
{
	std::unique_lock<std::mutex> lock(mtx);
	return pollingInterval;
}

void SteelBattalion::InputManager::setLEDState(uint32_t light, uint32_t intensity, bool refresh)
{
...
uint32_t SteelBattalion::InputManager::getLEDState(ControllerLEDEnum light)				// returns the LED brightness value from the rawLEDData byte array
{
...
uint32_t SteelBattalion::InputManager::getButtonState(ButtonEnum button) 					// returns the button state from the rawControlData byte array. It must use the button mask list to find where in the
...
void SteelBattalion::InputManager::refreshLEDs()								// This method sets the flag to write the LED data to the device when the poll method gets called again
{													// Multithreading makes everything better
	refreshLED.store(1, std::memory_order_release);
}

void SteelBattalion::InputManager::resetLEDs()								// Resets the LED values to 0
{
...
uint32_t SteelBattalion::InputManager::getAimingX()							// This method returns the integer value of the x axis of the aiming stick (the right joystick on the device)
{
	std::unique_lock<std::mutex> lock(mtx);
	return uint32_t(rawControlData[9])-127;									// Stored in byte 9 of the buffer
}

uint32_t SteelBattalion::InputManager::getAimingY()							// This method returns the integer value of the y axis of the aiming stick (the right joystick on the device)
{
	std::unique_lock<std::mutex> lock(mtx);
	return uint32_t(rawControlData[11])-127;									// Stored in byte 11 of the buffer
}

uint32_t SteelBattalion::InputManager::getRotationLever()							// This method returns the integer value of the rotation lever (the left joystick on the device)
{
	std::unique_lock<std::mutex> lock(mtx);
...
uint32_t SteelBattalion::InputManager::getSightChangeX()							// This method returns the integer value of the x axis of the sight change stick (the thumbstick on the device)
{
	std::unique_lock<std::mutex> lock(mtx);
...
uint32_t SteelBattalion::InputManager::getSightChangeY()							// This method returns the integer value of the y axis of the sight change stick (the thumbstick on the device)
{
	std::unique_lock<std::mutex> lock(mtx);
...
uint32_t SteelBattalion::InputManager::getLeftPedal()							// This method returns the integer value of the Left pedal
{
	std::unique_lock<std::mutex> lock(mtx);
	return uint32_t(rawControlData[19]);									// Stored in byte buffer 19
}

uint32_t SteelBattalion::InputManager::getMiddlePedal()							// This method returns the integer value of the Middle pedal
{
	std::unique_lock<std::mutex> lock(mtx);
	return uint32_t(rawControlData[21]);									// Stored in byte buffer 21
}

uint32_t SteelBattalion::InputManager::getRightPedal()							// This method returns the integer value of the Right pedal
{
	std::unique_lock<std::mutex> lock(mtx);
	return uint32_t(rawControlData[23]);									// Stored in byte buffer 23
}

uint32_t SteelBattalion::InputManager::getTunerDial()							// This method returns the integer value of the tuner dial
{
	std::unique_lock<std::mutex> lock(mtx);
	return uint32_t(rawControlData[24]) & 0x0F;								// Stored in byte buffer 24
}

uint32_t SteelBattalion::InputManager::getGearLever()						// this method returns the integer value of the gear lever
{
	std::unique_lock<std::mutex> lock(mtx);
...
uint32_t SteelBattalion::InputManager::getError()
{
	std::unique_lock<std::mutex> lock(mtx);
	return error;
}

SteelBattalion::InputManager::~InputManager()							// Destructor
{
